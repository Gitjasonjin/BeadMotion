// Adapted from the React Bits ThoughtLine source supplied by the user.
...
const fmt = ds => (ds < 600 ? `${(ds / 10).toFixed(1)} 秒` : `${Math.floor(ds / 600)} 分 ${((ds % 600) / 10).toFixed(1)} 秒`);
const spoken = ds =>
  ds < 600
    ? `${(ds / 10).toFixed(1)} 秒`
    : `${Math.floor(ds / 600)} 分 ${((ds % 600) / 10).toFixed(1)} 秒`;
          ...
          <div className="thought-line__fold">
            <div className="thought-line__steps">
              {steps.map((text, i) => {
                ...
                return (
                  <div key={i} className="thought-line__step" data-done={done ? '' : undefined}>
